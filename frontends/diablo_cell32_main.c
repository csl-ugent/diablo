...
#include <diabloppc.h>
#include <diablospe.h>
#include "diablo_cell32_options.h"
...
static void PutSpuBinary(t_spu_binary *sd, t_object *ppu_obj)
{
   ...
   *    - ppu-embedspu arguments (e.g. -m32)
   */
  char command[400];
  char embedname[100];
  sprintf(embedname, "%s-%d-embed32.o", TEMP_SPU_NAME, invokecount++);
  sprintf(command, "ppu-embedspu -m32 %s %s %s",
  ...
  t_string copiedhandle = StringConcat2("Copied",sd->handle);
  ...
  while ((rr = SECTION_REFED_BY(origsec)))
  {
    ...
    if (strcmp(RELOC_LABEL(rel), sd->handle) &&
        strcmp(RELOC_LABEL(rel), copiedhandle))
      FATAL(("spu handle is '%s', unexpected reloc with label '%s'; reloc is @R", sd->handle, RELOC_LABEL(rel), rel));
...
#if 0
  t_object *embed = ObjectGet(TEMP_SPU_NAME "-embed32.o", NULL, FALSE);
  t_section *sec;
  int i;
...
int main (int argc, char **argv)
{
  ...
  DiabloCell32OptionsInit ();
  OptionParseCommandLine (diablo_cell32_option_list, argc, argv, FALSE);
  OptionGetEnvironment (diablo_cell32_option_list);
  DiabloCell32OptionsVerify ();
  OptionDefaults (diablo_cell32_option_list);

  t_program_description *desc = ParseProgramDescriptionFile(diablo_cell32_options.program_description);
  ...
  diabloobject_options.linkerscript =
    StringDup(DATADIR "/linker_descriptions/ELF-ppc-BINUTILS_LD.ld");
