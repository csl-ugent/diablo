...
void ExtraOutput(const char* prefix, t_cfg* cfg, t_function* fun) {
  ...
  fprintf(extra_output_file, "%s,%p,%i,%i,%i,%i,%s\n", prefix, (void*)address_of_function(fun), bbls, insts, insts/bbls, couldbe_applied, FUNCTION_NAME(fun));
}

/* TODO: random BBL selection for flattening */
/* TODO: as usual: filtering with multiple transformations for the can and do transform that may or may not support the BBL, rather than a generic 'whatever the first one returns' */
/* TODO: if multiple obfuscators are returned, they should all be returned in the arraylist, so the random selection among them can be replayed */
#define GENERATE_DIVERSITY_BASE(basename, query, var, type, var_cantransform) \
  t_arraylist* Diversity ## basename ## CanTransform(t_cfg* cfg, t_function* fun, t_bbl* bbl, void** additional_info) { \
    auto vec = GetAllTypedTransformationsForType<type>(query); \
    if (vec.at(0)->canTransform(var)) { \
      return SimpleCanTransform(var_cantransform, NULL, 1); }\
    return NULL; \
  } \
  t_diversity_options Diversity ## basename ## DoTransform(t_cfg* cfg, t_function* fun, t_bbl* bbl, void* additional_info, t_randomnumbergenerator* rng) { \
    auto vec = GetAllTypedTransformationsForType<type>(query); /* TODO should come from additional_info then */ \
    vec.at(0)->doTransform(var, rng); \
    return diversity_options_null; \
  }

#define GENERATE_DIVERSITY_FUNCTION(basename, query) GENERATE_DIVERSITY_BASE(basename, query, fun, FunctionObfuscationTransformation, NULL)
#define GENERATE_DIVERSITY_BBL(basename, query) GENERATE_DIVERSITY_BASE(basename, query, bbl, BBLObfuscationTransformation, bbl)

GENERATE_DIVERSITY_BBL(FlipBranches, "flip_branch")
// GENERATE_DIVERSITY_BBL(OpaquePredicates, "opaquepredicate") /* This is not yet used by the iterative rules */
GENERATE_DIVERSITY_BBL(ThwartDisassembly, "branchfunction")
GENERATE_DIVERSITY_BBL(BranchFunctionBeforeAndAfterCalls, "splitcallwithbranchfunction")
GENERATE_DIVERSITY_BBL(SplitByTwoWayPredicate, "splittwoway")

// TODO TODO TODO
GENERATE_DIVERSITY_BBL(BranchFunctionBeforeAndAfterCallsGlobalVar, "splitcallwithbranchfunction")

GENERATE_DIVERSITY_FUNCTION(FlattenFunction, "flattenfunction:nomultiple")
GENERATE_DIVERSITY_FUNCTION(BranchFunctionInFirstBlock, "splitofffunctionhead")
...
/* TODO */
  ...
  ADD_TRANSFORMATION(DiversityBranchFunctionInFirstBlock, TRUE); /* TODO WTF this was FALSE? */
...
t_int32 DiversityEngineIterative(t_cfg * cfg)
{
  ...
  RNGSetRootGenerator(RNGCreateBySeed(diablodiversity_options.div_random_seed, "diversity_engine"));
