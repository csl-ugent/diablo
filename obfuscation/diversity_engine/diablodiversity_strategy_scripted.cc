...
void WriteScriptChoiceToFile(FILE* file, t_function* fun, t_script_choice* choice) {
  ...
  fprintf(file, "%p,%i,%i,%i,%i\n", (void*)address_of_function(fun), choice->seed, choice->reason, choice->max_cost, following);
...
void ReadScriptFile(const char* filename) {
  ...
  while (script && read_ok && !feof(script)) {
    ...
    for (i = 0; i < following; i++) {
      ...
      IGNORE_RESULT(fscanf(script, "%i,%s\n", &cost, transform_name));
...
void ReadReasonsForMapping(const char* filename, t_hash_table* insmap) {
  ...
  while(mapping && !feof(mapping)) {
    ...
    IGNORE_RESULT(fscanf(mapping, "%x,%i\n", &ins_addr, &why));
...
void InitStrategyScripted(t_cfg* cfg) {
  ...
  ASSERT(diablodiversity_options.div_iterative_rules && (strcmp(diablodiversity_options.div_iterative_rules, "") != 0), ("Iterative diversity requires a rule file"));
  ReadTransformationRules(diablodiversity_options.div_iterative_rules);
...
void AddRule(int reason, t_transformation_rule* rule) {
  ...
  VERBOSE(0, ("ADD RULE RULE: %i %s", reason, rule->transformation->name));
...
int TransformFunctionWithRules(t_cfg* cfg, t_function* fun, t_arraylist* rules, int seed, int max_cost, int iteration) {
  ...
  t_randomnumbergenerator* rng = RNGCreateBySeed(seed, "transformfunctionwithrules");
  ...
  } while(applied_transform && current_cost <= max_cost && !stop_after && possible_transforms->current_length > 0 /* defensive programming */ );
...
t_script_choice* InitCosts(int reason) {
  ...
  choice->seed = RNGGenerate(RNGGetRootGenerator());
